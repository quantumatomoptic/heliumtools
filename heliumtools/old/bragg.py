...
class Pulse:
    ...
    def modulation(self, t):
        if self.type == 1:
            ...
            return np.exp(+1j * OmegaD * (t - T / 2) / 2 + 1j * theta / 2) + np.exp(
                -1j * OmegaD * (t - T / 2) / 2 - 1j * theta / 2
            )

    def phase_drift(self, t):
        drift = (
            self.drift
        )  # value (in degrees) of the phase drift during the duration of the pulse
...
class Bragg:
    ...
    def set_params(self, params, pulse):
        ...
        self.detuning_list = np.linspace(
            self.min_detuning, self.max_detuning, self.number_of_detuning_points
        )
    ...
    def time_solver(self, Bragg_pulse):
        ...
        def zfunc2(z, t):
            z1, z2 = z
            return [
                z2 * 1j * pulse(t) * np.exp(1j * delta * t) / 2,
                z1 * 1j * np.conj(pulse(t)) * np.exp(-1j * delta * t) / 2,
            ]
        ...
        def zjac2(z, t):
            ...
            jac = np.array(
                [
                    [0, 1j * pulse(t) * np.exp(1j * delta * t) / 2],
                    [1j * np.conj(pulse(t)) * np.exp(-1j * delta * t) / 2, 0],
                ]
            )
            return jac
    ...
    def time_solver_5lvl(self, Bragg_pulse):
        ...
        def zfunc2(z, t):
            z1, z2, z3, z4, z5 = z
            return [
                z2 * 1j * pulse(t) * np.exp(1j * (delta + 4 * OmegaB) * t) / 2,
                z1 * 1j * np.conj(pulse(t)) * np.exp(-1j * (delta + 4 * OmegaB) * t) / 2
                + z3 * 1j * pulse(t) * np.exp(1j * (delta + 2 * OmegaB) * t) / 2,
                z2 * 1j * np.conj(pulse(t)) * np.exp(-1j * (delta + 2 * OmegaB) * t) / 2
                + z4 * 1j * pulse(t) * np.exp(1j * (delta) * t) / 2,
                z3 * 1j * np.conj(pulse(t)) * np.exp(-1j * (delta) * t) / 2
                + z5 * 1j * pulse(t) * np.exp(1j * (delta - 2 * OmegaB) * t) / 2,
                z4
                * 1j
                * np.conj(pulse(t))
                * np.exp(-1j * (delta - 2 * OmegaB) * t)
                / 2,
            ]
        ...
        def zjac2(z, t):
            ...
            jac = np.array(
                [
                    [
                        0,
                        1j * pulse(t) * np.exp(1j * (delta + 4 * OmegaB) * t) / 2,
                        0,
                        0,
                        0,
                    ],
                    [
                        1j
                        * np.conj(pulse(t))
                        * np.exp(-1j * (delta + 4 * OmegaB) * t)
                        / 2,
                        0,
                        1j * pulse(t) * np.exp(1j * (delta + 2 * OmegaB) * t) / 2,
                        0,
                        0,
                    ],
                    [
                        ...
                        * np.exp(-1j * (delta + 2 * OmegaB) * t)
                        / 2,
                        0,
                        1j * pulse(t) * np.exp(1j * (delta) * t) / 2,
                        0,
                    ],
                    [
                        0,
                        0,
                        1j * np.conj(pulse(t)) * np.exp(-1j * (delta) * t) / 2,
                        0,
                        1j * pulse(t) * np.exp(1j * (delta - 2 * OmegaB) * t) / 2,
                    ],
                    [
        ...
        # initial state
        z0 = np.array(
            [complex(0.0), complex(0.0), complex(1.0), complex(0.0), complex(0.0)]
        )
        # solve the equation
        z, infodict = odeintw(zfunc2, z0, self.time, Dfun=zjac2, full_output=True)
        # return the coefficients
    ...
    def compute(self, pulse):
        ...
        for k in tqdm(range(self.number_of_detuning_points), desc="Progress"):
            ...
            a_C2.append(
                np.angle(C2[self.number_of_points - 1])
                + self.detuning_list[k] * self.duration / 2 % (2 * pi)
            )
    ...
    def plot_boxes(self, plt):
        if self.show_boxes is True:
            for k in range(self.boxes_number + 1):
                plt.axvline(
                    x=-((2 * k + 1) / 2) * self.box_width / (2 * pi) * 1e-3,
                    color="black",
                    linestyle="--",
                    linewidth=1,
                )
            ...
            plt.axvspan(
                -self.box_width / (2 * pi) * (1e-3) / 2
                - self.boxes_number * self.box_width / (2 * pi) * (1e-3),
                -self.box_width / (2 * pi) * (1e-3) / 2,
                alpha=0.2,
                color="red",
            )
    ...
    def plot_reflectivity(self, results):
        ...
        for k in range(len(results)):
            ...
            plt.plot(
                self.detuning_list / (2 * pi) * 1e-3,
                P_C0,
                label=r"$|C_0|^2$" + label_curve,
            )
            plt.plot(
                self.detuning_list / (2 * pi) * 1e-3,
                P_C2,
                label=r"$|C_2|^2$" + label_curve,
            )
        plt.legend()
        plt.show()
    ...
    def plot_phase(self, results):
        ...
        for k in range(len(results)):
            P_C0, P_C2, a_C0, a_C2, label_curve = results[k]
            reflected_angle = (
                ((np.unwrap(4 * np.array(a_C2), discont=pi) / 4)) * 180 / pi
            )
            transmitted_angle = (
                ((np.unwrap(4 * np.array(a_C0), discont=pi) / 4)) * 180 / pi
            )
            ...
            plt.plot(
                self.detuning_list / (2 * pi) * 1e-3, imprinted_phase, label=label_curve
            )
            plt.legend()
        plt.show()
...
class Bell:
    def set_params(self, params_Bell, pulse_1, pulse_2):
        ...
        self.splitter_delay = (
            self.splitter_center
            - pulse_2.duration / 2
            - (self.mirror_delay + pulse_1.duration)
        )
        self.total_duration = (
            self.mirror_delay
            + self.mirror_duration
            + self.splitter_delay
            + self.splitter_duration
        )
        # absolute times
        self.t_start_mirror = self.mirror_delay
        self.t_stop_mirror = self.mirror_delay + self.mirror_duration
        self.t_start_splitter = (
            self.mirror_delay + self.mirror_duration + self.splitter_delay
        )
    ...
    def time_solver(self, initial_state, time):
        ...
        def zfunc2(z, t):
            ...
            return [
                -1j * omega0 * z1
                + z2 * 1j * pulse(t) * np.exp(1j * (delta + Delta_omega) * t) / 2,
                z1
                * 1j
                * np.conj(pulse(t))
                * np.exp(-1j * (delta + Delta_omega) * t)
                / 2
                - 1j * omega2 * z2,
            ]
        ...
        def zjac2(z, t):
            ...
            jac = np.array(
                [
                    [
                        -1j * omega0,
                        1j * pulse(t) * np.exp(1j * (delta + Delta_omega) * t) / 2,
                    ],
                    [
                        1j
                        * np.conj(pulse(t))
    ...
    def compute(self):
        ...
        if not self.remove_transmitted_atoms:
            for k in tqdm(range(self.number_of_detuning_points), desc="Progress"):
                ...
                Alice1_res.append(
                    [C0_Alice1[len(C0_Alice1) - 1], C2_Alice1[len(C0_Alice1) - 1]]
                )
                C0_Alice2, C2_Alice2 = self.time_solver(Alice2_init, self.time)
                Alice2_res.append(
                    [C0_Alice2[len(C0_Alice2) - 1], C2_Alice2[len(C0_Alice2) - 1]]
                )
                # Bob doublet
                self.detuning = +self.detuning_list[k]
                self.omega_0 = self.detuning_list[k]
        ...
        for k in range(len(self.detuning_list)):
            A.append(
                (1 / np.sqrt(2))
                * (
                    Alice1_res[k][0] * Bob1_res[k][0]
                    + np.exp(1j * phi0) * Alice2_res[k][0] * Bob2_res[k][0]
                )
            )
            B.append(
                (1 / np.sqrt(2))
                * (
                    Alice1_res[k][0] * Bob1_res[k][1]
                    + np.exp(1j * phi0) * Alice2_res[k][0] * Bob2_res[k][1]
                )
            )
            C.append(
                (1 / np.sqrt(2))
                * (
                    Alice1_res[k][1] * Bob1_res[k][0]
                    + np.exp(1j * phi0) * Alice2_res[k][1] * Bob2_res[k][0]
                )
            )
            D.append(
                (1 / np.sqrt(2))
                * (
                    Alice1_res[k][1] * Bob1_res[k][1]
                    + np.exp(1j * phi0) * Alice2_res[k][1] * Bob2_res[k][1]
                )
            )
    ...
    def plot_probabilities(self, results):
        ...
        plt.plot(
            self.detuning_list / (2 * pi) * 1e-3, Ppp, linestyle="--", label=r"$P_{++}$"
        )
        plt.plot(self.detuning_list / (2 * pi) * 1e-3, Ppm, label=r"$P_{+-}$")
        plt.plot(
            self.detuning_list / (2 * pi) * 1e-3, Pmp, linestyle="--", label=r"$P_{-+}$"
        )
    ...
    def plot_correlator(self, results):
        ...
        for k in range(len(results)):
            ...
            plt.plot(
                self.detuning_list / (2 * pi) * 1e-3, Bell_correlator, label=label_curve
            )
