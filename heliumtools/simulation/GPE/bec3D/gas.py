...
class Gas():
    ...
    Args:
        element (str): 
            Optional. The element the gas is made of. Defaults to "He*".
        N_particles (int): 
            Optional. The number of particles in the gas. Defaults to :math:`10^6`.
        N_grid (list of int): 
            Optional. The number of points on each side of the computational grid in each axis. Defaults to :math:`2^8`.
        grid_size (list of floats): 
            Optional. The side of the computational grid in meters in each axis. Defaults to :math:`10^{-6}`.
        device (torch.device or None): 
            Optional. The device where to store tensors. Defaults to None, meaning that GPU will be used if available.
        float_dtype (:py:attr:`torch.dtype`): 
            Optional. The dtype used to represent floating point numbers. Defaults to :py:attr:`torch.double`.
        complex_dtype (:py:attr:`torch.dtype`): 
            Optional. The dtype used to represent complex numbers. Defaults to :py:attr:`torch.complex128`.
        adimensionalization_length (float): 
            Optional. The unit of length in meters to be used during the simulations. Defaults to :math:`10^{-6}`.
    """

    def __init__(self, element: str = "He*", N_particles: int = int(1e6),
                 N_grid: list = [2**8,2**8,2**8], 
                 grid_size: list = [float(1e-6),float(1e-6),float(1e-6)],
                 device: Union[torch.device, None] = None, 
                 float_dtype: torch.dtype = torch.double, 
                 complex_dtype: torch.dtype = torch.complex128, 
                 adimensionalization_length: float = 1e-6) -> None:
        ...
        self.grid_size_x = grid_size[0] / self.adim_length
        #: float: The side of the computational grid along the y axis in adimensionalized units.
        self.grid_size_y = grid_size[1] / self.adim_length
        #: float: The side of the computational grid along the z axis in adimensionalized units.
        self.grid_size_z = grid_size[2] / self.adim_length
        #: int: The number of points on each side of the computational grid.
        self.N_grid = []
        for i in range(0,len(N_grid)):
            self.N_grid.append(int(N_grid[i]))
        ...
        self.x = torch.linspace(-self.grid_size_x/2, self.grid_size_x/2,
                                self.N_grid[0], dtype=self.float_dtype, device=self.device)
        ...
        self.y = torch.linspace(-self.grid_size_y/2, self.grid_size_y/2,
                                self.N_grid[1], dtype=self.float_dtype, device=self.device)
        #: torch.Tensor: The vector of adimensionalized grid coordinates along the :math:`z` axis.
        self.z = torch.linspace(-self.grid_size_z/2, self.grid_size_z/2,
                                self.N_grid[2], dtype=self.float_dtype, device=self.device)
        ...
        #: float: The distance between two consecutive points of the grid along the :math:`z` axis in adimensionalized units.
        ...
        #: torch.Tensor: The matrix of :math:`z` coordinates of the grid in adimensionalized units.
        ...
        self.kx = 2*np.pi * torch.fft.fftshift(torch.fft.fftfreq(
            self.N_grid[0] + 2 * (self.N_grid[0]//2), self.dx, dtype=self.float_dtype, device=self.device))
        ...
        self.ky = 2*np.pi * torch.fft.fftshift(torch.fft.fftfreq(
            self.N_grid[1] + 2 * (self.N_grid[1]//2), self.dy, dtype=self.float_dtype, device=self.device))
        #: torch.Tensor: The vector of adimensionalized momenta along the :math:`kz` axis.
        self.kz = 2*np.pi * torch.fft.fftshift(torch.fft.fftfreq(
            self.N_grid[2] + 2 * (self.N_grid[2]//2), self.dz, dtype=self.float_dtype, device=self.device))
        ...
        #: float: The distance between two consecutive points of the grid along the :math:`kz` axis in adimensionalized units.
        ...
        self.Ky = momenta[1]
        #: torch.Tensor: The matrix of :math:`kz` coordinates of the grid in adimensionalized units.
        self.Kz = momenta[2]
        del momenta
    ...
    def psi(self):
        ...
        if self._updated_wavefunction == UPDATED_PSIK:
            ...
            self.psi = iftn(self._psik)[
                self.N_grid[0]//2:self.N_grid[0]+self.N_grid[0]//2,
                self.N_grid[1]//2:self.N_grid[1]+self.N_grid[1]//2,
                self.N_grid[2]//2:self.N_grid[2]+self.N_grid[2]//2]
    ...
    def psik(self):
        ...
        if self._updated_wavefunction == UPDATED_PSI:
            ...
            self.psik = ftn(pad(self._psi, (self.N_grid[2]//2, self.N_grid[2]//2,
                            self.N_grid[1]//2, self.N_grid[1]//2,
                            self.N_grid[0]//2, self.N_grid[0]//2), mode="constant", value=0))
            self._updated_wavefunction = UPDATED_BOTH
        return self._psik
