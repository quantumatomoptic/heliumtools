...
class BecEvolution(Callback):
    """ Callback computing the the square of the wavefunction
    ...
    def on_epoch_end(self, epoch: int):
        """At the end of an epoch, if its number is a multiple of ``compute_every`` uses the stored wave function of the gas to compute 
        its density. If the epoch number is a multiple of ``print_every`` as well, the value of the norm is printed on screen.
