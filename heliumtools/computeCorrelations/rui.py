...
class Correlation1D(Correlation):
    ...
    def superimpose(self,ax1,ax2,):
        ...
        self.define_variable1(box="1",axe=ax1,type="position",name="V1",min = -middle - maxi,max = -middle + maxi + self.step_size,step = self.step_size)
        self.define_variable2(box="2",axe=ax2,type="position",name="V2",min = middle - maxi,max = middle + maxi + self.step_size,step = self.step_size)
    ...
    def compute_1D_correlation(self):
        ...
        roi_list = [{"V1-V2": {"position": -2 * middle, "size": 2*dW}},
                    {"V1+V2": {"position": -2 * middle, "size": 2*dW}},
                    {"V1+V2": {"position": 2 * middle, "size": 2*dW}}]
        ...
        for j, (attr_result, df_atoms, df_initial_res,axis_key) in enumerate(zip(
            ...
            df_initial_res["V1+V2"]=round(df_initial_res["V1"]+df_initial_res["V2"],3)
            df_initial_res["V1-V2"]=round(df_initial_res["V1"]-df_initial_res["V2"],3)
            # on dataframe with atoms
            df_atoms["V1+V2"] = round(df_atoms["V1"]+df_atoms["V2"],3)
            df_atoms["V1-V2"] = round(df_atoms["V1"]-df_atoms["V2"],3)

            # round values to avoid numerical problems
            df_initial_res["V1"] = np.round(df_initial_res["V1"],3)
            df_initial_res["V2"] = np.round(df_initial_res["V2"],3)
    ...
    def fit1DCorrelations(self):
        ...
        for j, (attr_result,axis_key,key) in enumerate(zip([self.cross_1Dresult, self.loc1_1Dresult, self.loc2_1Dresult],["V1+V2","V1-V2","V1-V2"],keys)):
    ...
    def showCorrelations(self,ax1,ax2):
        ...
        for j, (attr_result, df_atoms, df_initial_res,axis_key, axis_integrated) in enumerate(zip(
            ...
            for k in range(3):
                axes[k,j].plot(a["V1"],a["V2"],color = plt.get_cmap(cmaps[j])(1.0))
                axes[k,j].plot(b["V1"],b["V2"],color = plt.get_cmap(cmaps[j])(1.0))
            ...
            for jjj, Ystr in enumerate(["g^2"]):
                ...
                try: 
                    ...
                    legenda = "A0 = "+"{:.2f}({:.0f})".format(maxi,100*perr[0])+"\n"+r"$\sigma$ = "+"{:.2f}({:.0f})".format(popt[1],100*perr[1])
