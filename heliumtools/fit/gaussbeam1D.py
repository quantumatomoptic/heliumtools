...
def GaussianBeam1D(x, *p):
    return p[0] * np.exp(-2 * ((x - p[2]) ** 2) / p[1] ** 2)


# %% CLASS DEFINITION
class GaussianBeam1DFit(Abstract1DFit):
    """
    Class to fit datas : 1D Gaussian Beam fit

    ----------
    USAGE :
    oneDfit = GaussianBeam1DFit(x=x, z=z)
    oneDfit.do_guess()
    ...
    def __init__(self, **kwargs):
        ...
        self.category = "math"
        self.formula_help = "f(x) = "
        self.formula_help += "+ p[0] * exp(-2 * (x - p[2]) ** 2 / (p[1]**2))"
        self.parameters_help = "p = [amplitude, waist, center]"
        self._version = "1.0"

    def _fitfunc(self, x, *p):
        return GaussianBeam1D(x, *p)
    ...
    def do_guess(self):
        ...
        # -- guess amplitude / offset / center / waist
    ...
    def compute_values(self):
        ...
        # -- get fit results
        amplitude, waist, center = self.popt
        amplitude_err, waist_err, center_err = self.perr
...
if __name__ == "__main__":
    ...
    try:
        ...
        p = [0.5, 5.0, 0.5, 5]
        z = GaussianBeam1D(x, *p)
        noise = (np.random.rand(*x.shape) - 0.5) * 2
        z += noise

        # -- Fit
        g1Dfit = GaussianBeam1DFit(x=x, z=z)
        g1Dfit.do_guess()
        print(">> guess")
        print(g1Dfit.guess)
        g1Dfit.do_fit()
