...
def Lorentz(x, A, gamma, c):
    return A / (1 + (x - c) ** 2 / (gamma / 2) ** 2)


def Lorentz2D(xy, *p):
    """p = [offset, amplitude, size_x, size_y, center_x, center_y]"""
    (x, y) = xy
    return p[0] + p[1] * Lorentz(x, 1, p[2], p[4]) * Lorentz(y, 1, p[3], p[5])


# %% CLASS DEFINITION
class Lorentz2DFit(Abstract2DBellShaped):
    ...
    def __init__(self, **kwargs):
        ...
        self.formula_help += "+ p[1] /( 1+ ((x - p[4]) ** 2 / (p[2]/2)**2)))"
        self.formula_help += " /( 1+ ((x - p[5]) ** 2 / (p[3]/2)**2)))"
